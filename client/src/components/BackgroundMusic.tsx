...
export default function BackgroundMusic({compact=false}:{compact?:boolean}){
 ...
 return <div className={`music-dock ${playing?'is-playing':''} ${expanded?'is-expanded':''} ${compact?'is-compact':''}`}>
  ...
  <div className="music-mini"><button className="music-play" onClick={toggle} aria-label={playing||busy?'暫停背景音樂':'播放背景音樂'} title={playing?'暫停背景音樂':'播放背景音樂'}>{playing||busy?<Pause size={16}/>:<Play size={16}/>}</button><button className="music-track-info" aria-label="開啟音樂設定" aria-expanded={expanded} aria-controls="academy-music-settings" onClick={()=>setExpanded(v=>!v)}><span>毒蛇稱霸 <i className="music-equalizer" aria-hidden="true"><b/><b/><b/></i></span><small>{busy?'正在載入…':playing?'播放中 · 按左側可暫停':'音樂已關閉 · '+selected.label}</small></button><button className="music-settings-toggle" aria-label={expanded?'收起音樂播放器':'展開音樂播放器'} aria-expanded={expanded} onClick={()=>setExpanded(v=>!v)}><Music2 size={16}/></button></div>
